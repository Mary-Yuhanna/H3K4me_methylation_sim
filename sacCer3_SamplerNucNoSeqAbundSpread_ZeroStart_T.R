...
# Abundspread with fast nucleosome creation
# Here, attempting a genome/nucleosome sampler to accompany binding factor
# Need to add extra binding factors (one day may incorporate alongside profile/mods structure
# The sampler abundances control the number of hits, therefor the actual writer abundances
#   need to be near infinite/saturating.
# Need to alter layers to include extra sampled layers.
# Here, starting from zero methylation to see if w << e can still get to me3


#NOTES FOR DAVE
#MARY'S NOTES - first section creates an output directory to store output files from the simulation

simName <- "sacCer3_SamplerNucNoSeqAbundSpread_ZeroStart_T"
outputDir <- paste0("/Users/maryyuhanna/desktop/output_folder/", simName,"/")
...
source("/Users/maryyuhanna/desktop/startingResources/removeShortGRanges.R")
source("/Users/maryyuhanna/desktop/startingResources/removeGRangesBySize.R")
source("/Users/maryyuhanna/desktop/startingResources/plotLayers.R")
...
#bf_TF <-  createBindingFactor.DNA_consensus(name="bf_TF", patternString = "CAGCTG",
...
bf_meUp <- createBindingFactor.layer_region(name="bf_meUp", profile.layers=c("tf_active", "sampled_meUp"), 
                                            ...
                                            mod.marks=1,
                                            stateWidth= 6 + (4*nucleosomeWidth))
...
bfList_H3K4_methylate <- list( bf_meUp_sampler=bf_meUp_sampler, bf_meUp=bf_meUp, bf_promotion_0_1=bf_promotion_0_1, 
...
bf_methylate_abu <- c( 24000, rep(saturationAbundance, 5) )  
...
#matchBindingFactor.BSgenome(layerSet = scLayerSet, bindingFactor = bf_TF)  # ~ 5000 hits
#matchBindingFactor.BSgenome(layerSet = scLayerSet, bindingFactor = bf_meUp)
...
bf_demethylate_abu <- c( 0, rep(saturationAbundance, 4))  # setting the eraser to Zero for the burn-in.
names(bf_demethylate_abu) <- names(bfList_H3K4_demethyl)

#matchBindingFactor.BSgenome(layerSet = scLayerSet, bindingFactor = bf_meDown)    # should be many
#matchBindingFactor.BSgenome(layerSet = scLayerSet, bindingFactor = bf_demethylate_3_2)  # should be none yet
...
fileOut <- paste0(outputDir,"scLayerSetNuc.object.Rdata")
...
for(thisAbund in abundSpread)  {
  ...
  bfAbund_both_eFirst["bf_meDown"] <- thisAbund
  ...
  for(i in 1:100) { #MARY'S NOTES - running the simulation 100 times 
    ...
    if(i%%10 == 0)  {    # draw line plots of total coverage every tenth iteration, MARY'S NOTES - this plots the status of the BFs total coverage of me1, me2, me3  
      plot(scLayerSetBothAbund$history$nBlocks.H3K4me3, pch=6, ylab="nucleosomes", 
           xlab="iteration", main=paste("iteration", i), ylim=c(0,3500))
      ...
      plotLayers(scLayerSetBothAbund, layerNames=c("nucleosome","tf_active", "H3K4me1", "H3K4me2","H3K4me3"), chrom="chrVIII", xlim=c(495000,505000))
  ...
  fileOut <- paste0(outputDir,subSimName , ".history.tsv")
  write.table(scLayerSetBothAbund$history, file= fileOut, quote=F, sep="\t")
  
  fileOut <- paste0(outputDir,subSimName , ".nBlocks.png")
    png(filename = fileOut, width=2400, height=1600, res=300)
    plot(scLayerSetBothAbund$history$nBlocks.H3K4me3, pch=6, ylab="nucleosomes", 
       xlab="iteration", main=subSimName, ylim=c(0,3500))
    points(scLayerSetBothAbund$history$nBlocks.H3K4me2, pch=4)
    points(scLayerSetBothAbund$history$nBlocks.H3K4me1, pch=1)
    legend("topleft", legend=c("H3K4me3", "H3K4me2", "H3K4me1"), pch=c(6, 4, 1))
  dev.off() 
